...
const AddEventPreview: React.FC = () => {
  ...
  const handleShare = (ev: EventDataPreview) => {
    ...
    if (navigator.share) {
      navigator.share({
        title: ev.eventName,
        text: ev.eventDescription.replace(/<[^>]+>/g, ""),
        url,
      }).catch(() => {});
    } else {
        ...
        {events.map((ev, i) => (
          ...
          >
            <div
              className="ae-visual"
              style={{
                backgroundImage: `linear-gradient(rgba(10,12,20,0.18), rgba(10,12,20,0.02)), url(${ev.locationImage})`,
              }}
              role="img"
              aria-label={ev.eventName}
            >
              <div className="ae-visual-left">
                <span className={`ae-badge ae-badge-${ev.category.replace(/\s+/g, "").toLowerCase()}`}>
                  {ev.category}
                </span>
                <time className="ae-date">
                  <Calendar size={14} /> {fmt(ev.eventDate)}
                </time>
              </div>
            ...
            <div className="ae-body">
              <div className="ae-row">
                <div className="ae-head">
                  <h3 id={`ae-title-${ev.id}`} className="ae-name">
                    {ev.eventName}
                  </h3>
                  <div className="ae-meta">
                    <MapPin size={12} /> <span>{ev.eventLocation}</span>
                  </div>
                </div>
              ...
              <div className="ae-tiles">
                ...
                <div className="ae-tile">
                  ...
                  <div className="ae-tile-value">
                    {ev.openingDate && ev.closingDate ? `${fmt(ev.openingDate)} — ${fmt(ev.closingDate)}` : "—"}
                  </div>
                </div>
              ...
              <div className="ae-bottom">
                ...
                <div className="ae-share">
                  ...
                  <span className={`ae-toast ${copied === ev.id ? "show" : ""}`}>
                    {copied === ev.id ? "Copied!" : ""}
                  </span>
                </div>
              </div>
