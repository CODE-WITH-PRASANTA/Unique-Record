...
const AchivmentDetails = () => {
  ...
  // Fetch achievement details and latest achievements
  useEffect(() => {
  ...
  const toggleReadMoreContent = () => setReadMoreContent(!readMoreContent);
  ...
  const renderTruncatedContent = (content) => {
    ...
    if (textContent.length <= 500) return content;
    return readMoreContent ? content : textContent.slice(0, 500) + "...";
  };
        ...
        <div className="Achivments-details-content-area">
          ...
          {achivmentDetails.uruHolderLink && (
            <div className="record-holder-link">
              <a href={achivmentDetails.uruHolderLink} target="_blank" rel="noopener noreferrer" className="arrow-button">
          ...
          <div className="Achivments-details-paragraph">
            <div dangerouslySetInnerHTML={{ __html: renderTruncatedContent(achivmentDetails.content) }} />
            {achivmentDetails.content && achivmentDetails.content.length > 500 && (
              <button className="read-more-btn" onClick={toggleReadMoreContent}>
                {readMoreContent ? "Show Less" : "Read More"}
              </button>
            )}
          </div>
            ...
            <div className="Achivments-details-tags-row">
              ...
              <div className="social-share">
                ...
                <button
                  ...
                  onClick={() =>
                    window.open(
                      `https://twitter.com/intent/tweet?text=${encodeURIComponent(`${achivmentDetails.title}\n\n©Provider: ${achivmentDetails.providerName}\n\nRead link 👇\n${window.location.href}`)}`,
                      "_blank"
                    )
                  }
                >
                ...
                <button
                  ...
                  onClick={() =>
                    window.open(
                      `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}&summary=${encodeURIComponent(`${achivmentDetails.title}\n\n©Provider: ${achivmentDetails.providerName}\n\nRead link 👇\n${window.location.href}`)}`,
                      "_blank"
                    )
                  }
                >
        ...
        <aside className="Achivments-details-sidebar">
          ...
          <div className="Achivments-details-categories">
            ...
            <div className="Achivments-details-category-tags">
              {categories.slice(0, showAllCategories ? categories.length : 5).map((category, index) => (
                <span key={index} className="Achivments-details-category-tag">{category}</span>
              ))}
            </div>
            {categories.length > 5 && (
          ...
          <div className="Achivments-details-tags-sidebar">
            ...
            <div className="Achivments-details-tags">
              {tags.map((tag, index) => (
                <span key={index} className="Achivments-details-tag">{tag}</span>
              ))}
            </div>
          </div>
            ...
            {loadingComments ? (
              ...
              <div className="AchivmentCmt-list">
                {comments.map((comment) => {
                  ...
                  const truncatedMessage = comment.message.length > 120 ? comment.message.substring(0, 120) + "..." : comment.message;
                  return (
                    <div key={comment._id} className="AchivmentCmt-item">
                      <p className="AchivmentCmt-field"><span>Name:</span> {comment.name}</p>
                      <p className="AchivmentCmt-field"><span>Email:</span> {comment.email}</p>
                      <p className="AchivmentCmt-field"><span>Address:</span> {comment.address}</p>
                      <p className="AchivmentCmt-message">{expanded ? comment.message : truncatedMessage}</p>
                      {comment.message.length > 120 && (
                        <button className="AchivmentCmt-readmore" onClick={() => toggleReadMore(comment._id)}>
